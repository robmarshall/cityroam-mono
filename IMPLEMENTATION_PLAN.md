...
- [ ] **1.0 Docker compose for local dev** — postgres + redis services, shared network, .env loading. Developers need DB/Redis running before anything else → Spec 10 §10.1 (partial — just postgres + redis initially, full compose in 9.1)
- [ ] **1.1 Monorepo & root config** — npm workspaces, tsconfig.base.json, root scripts, .env.example (including REVIEW_LINK), .gitignore, .nvmrc, package scaffolding for all 5 packages with correct exports config → Spec 01 §1.1-1.2
