...
export function authorRouteText(
  ...
  return [
    ...
    `   The guide is the Owl (in this language: "${guideNameFor(args.language)}"). It introduces itself once, in the introduction group, with the {{GUIDE_NAME}} template variable ("I'm {{GUIDE_NAME}}.") — never hard-code the name. At most one owl reference in the whole route, and no owl puns (guide-personality.md > The Owl).`,
...
export function translateRouteText(config: Config, args: { route_id: string; target_language: string }): string {
  return [
    ...
    `   The guide is the Owl; in "${args.target_language}" {{GUIDE_NAME}} becomes "${guideNameFor(args.target_language)}". Keep the variable where the source has it (the intro's "I'm {{GUIDE_NAME}}."), make first-person lines agree with the name's grammatical gender (translation-guide.md > The Guide's Name), and never add or translate an owl pun.`,
