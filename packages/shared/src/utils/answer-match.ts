/**
 * Answer normalisation and lexical matching, shared by the API's pipeline
 * (the deterministic checks before and after the LLM) and the marketing
 * site's scripted "Try the Owl" demo, so both accept the same answers.
 *
 * Framework-free and runtime-dependency-free: the only import is a type.
 *
 * - `deterministicAnswerMatch`: strict whole-word match with article
 *   stripping and a negation guard ("definitely not the town hall" is not a
 *   correct answer).
 * - `nearAnswerMatch`: the looser, still purely lexical check that tolerates
 *   a couple of typos and common abbreviations.
 * - `containsPhraseFromList` / `matchesWordList`: word-list checks (hint and
 *   skip requests, yes and no).
 */
 ...
 */
export function tokenizeAnswer(s: string): string[] {
...
/** Escape a normalised phrase for use inside a RegExp. */
 ...
 */
export function containsPhraseFromList(text: string, phrases: readonly string[]): boolean {
 ...
 * and check it equals one of the words.
 */
export function matchesWordList(text: string, words: readonly string[]): boolean {
  ...
  return words.some((w) => normalised === stripAccents(w).toLowerCase());
}
 ...
 * counts too.
 */
function isNegated(tokens: readonly string[], start: number, negations: ReadonlySet<string>): boolean {
...
export function deterministicAnswerMatch(
  ...
  const tokens = stripLeadingArticles(tokenizeAnswer(userMessage), articles);
  if (tokens.length === 0) return false;

  for (const answer of acceptedAnswers) {
    const answerTokens = stripLeadingArticles(tokenizeAnswer(answer), articles);
...
export function nearAnswerMatch(
  ...
  const rawTokens = stripLeadingArticles(tokenizeAnswer(userMessage), articles);
  if (rawTokens.length === 0) return false;
  const tokens = expandAbbreviations(rawTokens);

  for (const answer of acceptedAnswers) {
    const answerTokens = expandAbbreviations(stripLeadingArticles(tokenizeAnswer(answer), articles));
    if (answerTokens.length === 0) continue;
